...
def growth_index(cluster, total_doc, max_year, min_year):
    ...
    for x in range(0, data_collection_period):
        ...
        current_year_count = len(
            cluster[cluster["Year"] <= up_to_current_year])
        previous_year_count = len(
            cluster[cluster["Year"] <= up_to_previous_year])
        if (previous_year_count > 0):
            sum_of_growth = sum_of_growth + \
                ((current_year_count - previous_year_count) / previous_year_count)
        else:
            break

    growth_index = cluster_percentage * \
        (sum_of_growth / ((data_collection_period - 1) * 100))
    return growth_index
